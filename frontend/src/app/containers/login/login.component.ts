...
export class LoginComponent implements OnInit { 
  ...
  constructor(public userService: UserService, public router: Router) { } // Tengo que poner aquí el Router para poder utilizar el redirectRoute más abajo
    ...
    this.userService.login(this.user) // Mando el usuario al userService, a la funcióno de login
      .subscribe( // Lo subscribo y cuando llega la respuesta: 
        (res: HttpResponse<any>) => {
          ...
          const redirectRoute = admins.includes(res['user']['role']) ? '/products':'/'; // Si la respuesta incluye los roles de admin o superadmin guardame la ruta "/admin" en la constante redirectRoute, si no, guardame "/"
          ...
          this.userService.setUser(res['user']); // Guardo el user en el userService
          this.userService.setToken(res['token']); // Guardo el token en el userService. 
          ...
          setTimeout(() => this.router.navigate([redirectRoute]) , 1500); // y aquí le pido que navegue a la ruta guardada en redirectRoute
        },
